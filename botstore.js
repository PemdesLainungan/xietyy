require('./settings')
const { default:  pshkntkConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require('@adiwajshing/baileys')
...
return status
 ...
 pshkntk.ev.on('connection.update', (update) => {
const {connection,lastDisconnect} = update
if (connection === 'close') {lastDisconnect.error?.output?.statusCode !== DisconnectReason.loggedOut ? startpshkntk() : ''}
else if(connection === 'open') { pshkntk.sendMessage(`${nomerOwner}@s.whatsapp.net`, {text:`${JSON.stringify(update, undefined, 2)}`})}
console.log(update)})
